...
import { TodoSchema, todoSchema } from '../lib/types';
...
export const TodoForm = () => {
  ...
  } = useForm<TodoSchema>({ resolver: zodResolver(todoSchema) });
  ...
  const newTodo = useMutation({
    ...
    onError: (err, _, context) => {
      ...
      toast(`${error.message} Please Try Later`);
    },
  });

  const onSubmit: SubmitHandler<TodoSchema> = async (data: TodoSchema) => {
    newTodo.mutate({ ...data, id: crypto.randomUUID() });
      ...
      <div className="flex flex-col">
        <div>
          ...
          <button
            ...
            type="submit"
            className="bg-blue-500 disabled:bg-gray-500 py-2 rounded px-4"
          >
        ...
        <div>
          {errors.description && <span className="text-red-600">{`${ERROR_MESSAGES.description}`}</span>}
        </div>
      </div>
    </form>
  );
