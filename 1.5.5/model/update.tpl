...
func (m *default{{.upperStartCamelObject}}Model) UpdateOne(ctx context.Context,session sqlx.Session, {{if .containsIndexCache}}newData{{else}}data{{end}} *{{.upperStartCamelObject}})  (*{{.upperStartCamelObject}},error) {
  ...
  var resp {{.upperStartCamelObject}}
	...
	query := fmt.Sprintf("update %s set %s where {{.originalPrimaryKey}} = {{if .postgreSql}}$1{{else}}?{{end}} returning *", m.table, {{.lowerStartCamelObject}}RowsWithPlaceHolder)
	if session != nil{
	...
	if session != nil{
		return nil, session.QueryRow, Ctx(ctx, &resp, query, {{.expressionValues}})
	}
	nil, err:= m.conn.QueryRowCtx(ctx, &resp, query, {{.expressionValues}}){{end}}
  return &resp, err
}
